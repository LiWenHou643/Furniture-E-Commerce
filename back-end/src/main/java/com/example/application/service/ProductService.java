...
public class ProductService {
	...
	public Page<ProductDTO> getProducts(List<String> categories, List<String> brands, List<String> materials,
			Double minPrice, Double maxPrice, Integer minRating, String keyword, Pageable pageable) {
		...
		if (keyword != null && !keyword.trim().isEmpty()) {
			...
			products = products.stream()
					.filter(product -> product.getProductName().toLowerCase().contains(lowerCaseKeyword)).toList();
		}
	...
	@Caching(evict = { @CacheEvict(cacheNames = PRODUCT_LIST_CACHE_KEY, allEntries = true) }, put = {
	...
	public ProductDTO addProduct(ProductDTO productDTO) throws IOException {
		...
		Material material = materialRepository.findById(productDTO.getMaterialId())
				.orElseThrow(() -> new ResourceNotFoundException("Material", "id", productDTO.getMaterialId()));
		...
		product.getProductItems().forEach(pi -> {
			...
			pi.setProductImages(pi.getProductImages().stream().peek(piImage -> piImage.setProductItem(pi)) // Set the
																											// product
																											// item for
																											// each
																											// image
